...
namespace AsaludEcopetrol.Controllers.Censo
{
    ...
    public class CensoController : Controller
    {
        ...
        public ActionResult TableroAhAseguramiento(int? regional, int? unis, int? municipio)
        {
            ...
            List<management_censo_aseguramientoTableroControlResult> listadoCF = new List<management_censo_aseguramientoTableroControlResult>();
            List<management_censo_aseguramientoTableroControlResult> listadogestionado = new List<management_censo_aseguramientoTableroControlResult>();
            List<management_censo_aseguramientoTableroControlResult> listadoSinGestion = new List<management_censo_aseguramientoTableroControlResult>();

            try
            {
                listado = BusClass.DatosCensoAseguramiento(regional, unis, municipio);

                listadoCF = listado.Where(x => x.estado == 3).ToList();
                listadogestionado = listado.Where(x => x.estado == 2).ToList();
                listadoSinGestion = listado.Where(x => x.estado == 1).ToList();
            ...
            ViewBag.listadoCF = listadoCF;
            ViewBag.listadogestionado = listadogestionado;
            ViewBag.listadoSinGestion = listadoSinGestion;

            ViewBag.conteoCF = listadoCF.Count();
            ViewBag.conteoGestionados = listadogestionado.Count();
            ViewBag.conteoSinGestion = listadoSinGestion.Count();
