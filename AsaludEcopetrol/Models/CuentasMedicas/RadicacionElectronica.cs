...
namespace AsaludEcopetrol.Models.CuentasMedicas
{
    public class RadicacionElectronica
    {
        ...
        public string EnvioCambioEstadoEcopetrol(int? idFactura, string motivoCausaPrincipal)
        {
            var respuesta = "";
            management_API_consultaFactura_idAfResult factura = new management_API_consultaFactura_idAfResult();

            try
            {
                factura = BusClass.TraerDatosFacturaIdFac(idFactura);

                var IdVIM = !string.IsNullOrEmpty(factura.idVIM) ? factura.idVIM : "0";
                var numeroFactura = factura.num_factura;
                var codigoSAP = factura.codigo_SAP;
                var fechaEstado = DateTime.Now.ToString("yyyy-MM-dd");
                var estado = factura.estado_factura;

                // URL de generación del token
                var tokenUrl = "https://wssapvimpruebas.aplicativoasalud.co/api/Authentication/GenerarToken?user=ECOPETROL_PRUEBAS&pass=Sup3rU53r*_1";
                ...
                using (var clientToken = new HttpClient())
                {
                    ...
                    var jsonToken = responseToken.Content.ReadAsStringAsync().Result;
                    var tokenObj = Newtonsoft.Json.Linq.JObject.Parse(jsonToken);
                    token = tokenObj["token"]?.ToString(); // Aquí es "token" (no "access_token")
                }

                // Construir URL de envío
                var url = $"https://wssapvimpruebas.aplicativoasalud.co/api/RadicacionElectronica/ActualizacionEstadoEcopetrol" +
                          $"?IdVIM={IdVIM}&numeroFactura={numeroFactura}&codigoSAP={codigoSAP}" +
                          $"&fechaEstado={fechaEstado}&estado={estado}&motivoCausaPrincipal={motivoCausaPrincipal}";
                ...
                using (var client = new HttpClient())
                {
                    ...
                    // Importante: enviar POST aunque el body esté vacío
                    var emptyContent = new StringContent("", System.Text.Encoding.UTF8, "application/json");
                    var response = client.PostAsync(url, emptyContent).Result;

                    if (!response.IsSuccessStatusCode)
                    {
                        respuesta = $"Error al consumir API: {response.StatusCode}";
                    }
                    else
                    {
                        var json = response.Content.ReadAsStringAsync().Result;
                        respuesta = $"Respuesta del servicio: {json}";
                    }
                }
            }
            catch (Exception ex)
        ...
        public string EnvioArchivoHES(FileContentResult archivo)
        {
            var respuesta = "";
            ...
            try
            {
                ...
                using (var clientToken = new HttpClient())
                {
                    ...
                    var jsonToken = responseToken.Content.ReadAsStringAsync().Result;
                    var tokenObj = JObject.Parse(jsonToken);
                    token = tokenObj["token"]?.ToString(); // Aquí es "token"
                }

                // Paso 2: Enviar archivo al endpoint interno RecepcionArchivos
                var url = "https://wssapvimpruebas.aplicativoasalud.co/api/RadicacionElectronica/RecepcionArchivos";
                ...
                using (var content = new MultipartFormDataContent())
                {
                    ...
                    // El nombre "file" debe coincidir con el nombre del parámetro en RecepcionArchivos
                    content.Add(fileContent, "file", archivo.FileDownloadName);
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        respuesta = $"Error al consumir API: {response.StatusCode} - {responseContent}";
                    }
                    else
                    {
                        respuesta = $"Respuesta del servicio: {responseContent}";
                    }
                }
            }
            catch (Exception ex)
