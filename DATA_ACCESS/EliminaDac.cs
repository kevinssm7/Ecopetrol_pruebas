    ...
    public class EliminaDac
    {
        ...
        public int EliminarRecomendacionAuditoria(int? idRecomendacion)
        {
            try
            {
                using (ECOPETROL_DataContexDataContext db = new ECOPETROL_DataContexDataContext())
                {
                    creacion_recomendaciones_auditoria obj = db.creacion_recomendaciones_auditoria.FirstOrDefault(x => x.id_recomendacion == idRecomendacion);
                    db.creacion_recomendaciones_auditoria.DeleteOnSubmit(obj);
                    db.SubmitChanges();
                    return 1;
                }
            }
